test_that("simulate_temp_shifts builds scenarios from best model", {
  ...
  sims <- simulate_temp_shifts(fit, deltas = 1:2, as_long = FALSE)

  expect_true(all(c("day_of_year","baseline","Temp+1C","Temp+2C") %in% names(sims)))
  # shifts are additive
  i <- 100
...
test_that("simulate_temp_shifts maps to supplied dates", {
  ...
  sims <- simulate_temp_shifts(fit, deltas = c(1,5), dates = days, as_long = TRUE)

  expect_true(all(c("date","key","temp_c") %in% names(sims)))
  expect_setequal(levels(sims$key), c("baseline","Temp+1C","Temp+5C"))
